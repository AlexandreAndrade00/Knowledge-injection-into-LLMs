...
class PLM:
    ...
    def __init__(
        ...
        formatter: Callable[[str, str], object],
        model_name: str,
    ):
    ...
    def inference(self, model_input: str, context: str) -> str:
        ...
        return self.__model(formated_input, max_new_tokens=2048)[0]["generated_text"][
            -1
        ]["content"]

    def benchmark(
        self, dataset_w_context: Iterable[dict[str, str]]
    ) -> Iterable[dict[str, str]]:
        for elem in dataset_w_context:
            context: str = elem.get("context", "")

            result = self.__model(
                self.__input_formatter(elem["question"], context), max_new_tokens=2048
